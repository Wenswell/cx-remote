...
- Web adoption imports the native Codex transcript into Hub messages and records the latest transcript activity snapshot before opening the session.
- CLI/API adoption registers an explicit Codex thread id under Hub control on the selected node, then records the latest transcript activity snapshot when that thread exists in local Codex storage. Add `--import` or `importTranscript: true` to import the stored transcript into Hub messages.
- Runtime startup resumes an adopted or previously persisted Codex thread before starting the next turn.
- External native Codex CLI activity is tracked separately from the Hub-managed session status. Hooks update `nativeCodexActivity` by Codex thread id before and after adoption; after adoption the same row is associated through the matching `codexThreadId`. The Hub-owned `Session.status` keeps its existing managed-runtime meaning.
- Existing unexpired hook activity takes priority during adoption. When no live hook activity exists, adoption snapshots map an unfinished transcript turn to `working`, `task_complete` to `idle`, `turn_aborted` to `idle`, and `task_failed` to `error`. A `working` snapshot starts a normal 60 second lease and later hooks continue the state.
