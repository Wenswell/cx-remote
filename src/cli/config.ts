...
import { findSettingField, type SettingField } from '../config/fields.js';
...
export function runConfigCommand(argv: string[]): void {
  ...
  if (subcommand === 'set') {
    ...
    if (field.restartRequired) console.log('restart required');
    return;
  }
...
function printFields(revealSecrets: boolean): void {
  const settings = readSettings();
...
function printValue(field: SettingField, value: unknown, revealSecrets: boolean): void {
