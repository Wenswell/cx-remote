...
import { z } from 'zod';
import { ClusterService, LOCAL_NODE_ID } from '../cluster/service.js';
import { resolveCodexRuntimeDefaults } from '../agents/codex/defaults.js';
import { getSettingValue, isPathInside, listSettingFields, maskSettings, serverBasePath, setSettingValue } from '../config/config.js';
...
export class HubServer {
    ...
    server = null;
    cluster;
    constructor(hub, config, options = {}) {
        this.hub = hub;
        this.config = config;
        this.options = options;
        this.cluster = new ClusterService(this.hub, this.hub.events, this.config, this.options.codexHome, this.options.fetchImpl);
    }
    async start() {
        if (this.server)
            return;
    ...
    createApp() {
        ...
        app.get(route('/api/files'), async (c) => {
            ...
            if (workspaceId)
                return c.json(await this.cluster.listFiles(workspaceId, path));
        ...
        app.get(route('/api/events'), (c) => {
            ...
            const stream = new ReadableStream({
                start: (controller) => {
                    ...
                    const send = (data) => controller.enqueue(encoder.encode(encodeSseFrame(data)));
                    for (const event of this.hub.listEvents(afterId, sessionId)) {
                        if (localOnly && isRelayedEvent(event))
                            continue;
                        send(event);
                    }
...
function renderWebIndex(webDistDir, basePath) {
...
function webAuthMiddleware(token, publicUrl, basePath) {
    return async (c, next) => {
...
function webAuthResult(c, token, publicUrl, basePath) {
...
function isWebPageRequest(path, method, accept) {
...
function isAuthorizedRequest(authorization, cookie, token) {
    return authorization === `Bearer ${token}` || authCookieValue(cookie) === token;
}
function authCookieValue(header) {
    if (!header)
        return undefined;
    for (const part of header.split(';')) {
        const [name, ...value] = part.trim().split('=');
        if (name === WEB_AUTH_COOKIE)
            return decodeURIComponent(value.join('='));
    }
    return undefined;
}
function authCookie(token, secure, basePath) {
    return [
...
function isSecureCookie(publicUrl) {
...
function promptJobStatuses(value) {
    if (value === 'all')
        return undefined;
    if (value === 'active')
...
function errorStatus(error) {
    ...
    if (message.includes('not found'))
        return 404;
    if (message.includes('already running') || message.includes('controlled by') || message.includes('already managed'))
        return 409;
...
function workspaceRoot(roots, input) {
    if (roots.length === 0)
...
function aggregateNodeStats(nodes) {
...
function isLocalScope(value) {
    return value === 'local';
}
function isRelayedEvent(event) {
