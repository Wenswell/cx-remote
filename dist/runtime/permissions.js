...
export class PermissionService {
    ...
    config;
    pending = new Map();
    constructor(store, events, config) {
        this.store = store;
        this.events = events;
        this.config = config;
    }
    async requestApproval(sessionId, toolName, input) {
    ...
    async requestChoice(sessionId, input) {
    ...
    async resolveApproval(approvalId, decision, source, reason) {
    ...
    expireSessionApprovals(sessionId, reason) {
...
function approvalResponse(decision, reason) {
    if (decision === 'approved' || decision === 'allow')
        return { decision: 'approved' };
    if (decision === 'approved_for_session' || decision === 'allow_session')
        return { decision: 'approved_for_session' };
    if (decision === 'abort')
        return { decision: 'abort', reason };
    return { decision: 'denied', reason };
}
function choiceResponse(pending, decision) {
    if (!pending || pending.type !== 'choice')
        return { decision: 'cancel' };
    if (decision === 'cancel' || decision === 'denied' || decision === 'deny')
        return { decision: 'cancel' };
...
function extractChoiceOptions(input) {
    ...
    for (let questionIndex = 0; questionIndex < questions.length; questionIndex++) {
        ...
        for (const candidate of candidates) {
            ...
            const value = record
                ...
                : asString(candidate);
            if (label && value)
                out.push({ label, value, questionId });
        }
    }
    if (out.length === 0)
        out.push({ label: 'Continue', value: 'continue', questionId: 'answer' });
    return out;
}
