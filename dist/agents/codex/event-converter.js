...
export class CodexEventConverter {
    ...
    convertWrapped(params) {
        ...
        if (type === 'task_started')
            return [{ type: 'task_started', turn_id: asString(msg.turn_id ?? msg.turnId) }];
        if (type === 'task_complete')
            return [{ type: 'task_complete', turn_id: asString(msg.turn_id ?? msg.turnId) }];
        if (type === 'turn_aborted')
            return [{ type: 'turn_aborted', turn_id: asString(msg.turn_id ?? msg.turnId) }];
        if (type === 'task_failed')
            return [{ type: 'task_failed', error: asString(msg.error ?? msg.message) }];
