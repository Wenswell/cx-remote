...
`,K=class extends N{constructor(){super(...arguments),this.formControlController=new bn(this,{assumeInteractionOn:[`sl-blur`,`sl-input`]}),this.hasSlotController=new Yt(this,`help-text`,`label`),this.hasFocus=!1,this.title=``,this.name=``,this.value=``,this.size=`medium`,this.filled=!1,this.label=``,this.helpText=``,this.placeholder=``,this.rows=4,this.resize=`vertical`,this.disabled=!1,this.readonly=!1,this.form=``,this.required=!1,this.spellcheck=!0,this.defaultValue=``}get validity(){return this.input.validity}get validationMessage(){return this.input.validationMessage}connectedCallback(){super.connectedCallback(),this.resizeObserver=new ResizeObserver(()=>this.setTextareaHeight()),this.updateComplete.then(()=>{this.setTextareaHeight(),this.resizeObserver.observe(this.input)})}firstUpdated(){this.formControlController.updateValidity()}disconnectedCallback(){var e;super.disconnectedCallback(),this.input&&((e=this.resizeObserver)==null||e.unobserve(this.input))}handleBlur(){this.hasFocus=!1,this.emit(`sl-blur`)}handleChange(){this.value=this.input.value,this.setTextareaHeight(),this.emit(`sl-change`)}handleFocus(){this.hasFocus=!0,this.emit(`sl-focus`)}handleInput(){this.value=this.input.value,this.emit(`sl-input`)}handleInvalid(e){this.formControlController.setValidity(!1),this.formControlController.emitInvalidEvent(e)}setTextareaHeight(){this.resize===`auto`?(this.sizeAdjuster.style.height=`${this.input.clientHeight}px`,this.input.style.height=`auto`,this.input.style.height=`${this.input.scrollHeight}px`):this.input.style.height=``}handleDisabledChange(){this.formControlController.setValidity(this.disabled)}handleRowsChange(){this.setTextareaHeight()}async handleValueChange(){await this.updateComplete,this.formControlController.updateValidity(),this.setTextareaHeight()}focus(e){this.input.focus(e)}blur(){this.input.blur()}select(){this.input.select()}scrollPosition(e){if(e){typeof e.top==`number`&&(this.input.scrollTop=e.top),typeof e.left==`number`&&(this.input.scrollLeft=e.left);return}return{top:this.input.scrollTop,left:this.input.scrollTop}}setSelectionRange(e,t,n=`none`){this.input.setSelectionRange(e,t,n)}setRangeText(e,t,n,r=`preserve`){let i=t??this.input.selectionStart,a=n??this.input.selectionEnd;this.input.setRangeText(e,i,a,r),this.value!==this.input.value&&(this.value=this.input.value,this.setTextareaHeight())}checkValidity(){return this.input.checkValidity()}getForm(){return this.formControlController.getForm()}reportValidity(){return this.input.reportValidity()}setCustomValidity(e){this.input.setCustomValidity(e),this.formControlController.updateValidity()}render(){let e=this.hasSlotController.test(`label`),t=this.hasSlotController.test(`help-text`),n=this.label?!0:!!e,r=this.helpText?!0:!!t;return w`
    ...
    `}};K.styles=[k,qn,Zi],D([M(`.textarea__control`)],K.prototype,`input`,2),D([M(`.textarea__size-adjuster`)],K.prototype,`sizeAdjuster`,2),D([j()],K.prototype,`hasFocus`,2),D([A()],K.prototype,`title`,2),D([A()],K.prototype,`name`,2),D([A()],K.prototype,`value`,2),D([A({reflect:!0})],K.prototype,`size`,2),D([A({type:Boolean,reflect:!0})],K.prototype,`filled`,2),D([A()],K.prototype,`label`,2),D([A({attribute:`help-text`})],K.prototype,`helpText`,2),D([A()],K.prototype,`placeholder`,2),D([A({type:Number})],K.prototype,`rows`,2),D([A()],K.prototype,`resize`,2),D([A({type:Boolean,reflect:!0})],K.prototype,`disabled`,2),D([A({type:Boolean,reflect:!0})],K.prototype,`readonly`,2),D([A({reflect:!0})],K.prototype,`form`,2),D([A({type:Boolean,reflect:!0})],K.prototype,`required`,2),D([A({type:Number})],K.prototype,`minlength`,2),D([A({type:Number})],K.prototype,`maxlength`,2),D([A()],K.prototype,`autocapitalize`,2),D([A()],K.prototype,`autocorrect`,2),D([A()],K.prototype,`autocomplete`,2),D([A({type:Boolean})],K.prototype,`autofocus`,2),D([A()],K.prototype,`enterkeyhint`,2),D([A({type:Boolean,converter:{fromAttribute:e=>!(!e||e===`false`),toAttribute:e=>e?`true`:`false`}})],K.prototype,`spellcheck`,2),D([A()],K.prototype,`inputmode`,2),D([Kn()],K.prototype,`defaultValue`,2),D([O(`disabled`,{waitUntilFirstUpdate:!0})],K.prototype,`handleDisabledChange`,1),D([O(`rows`,{waitUntilFirstUpdate:!0})],K.prototype,`handleRowsChange`,1),D([O(`value`,{waitUntilFirstUpdate:!0})],K.prototype,`handleValueChange`,1),K.define(`sl-textarea`);var Qi=600*1e3,$i=new URLSearchParams(location.search).get(`token`)||``;$i&&Pa();var ea=`cx_remote_notify_sessions`,ta=[`<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">`,`<path fill-rule="evenodd" d="M8 3a5 5 0 1 0 4.546 2.914.5.5 0 0 1 .908-.417A6 6 0 1 1 8 2z"/>`,`<path d="M8 4.466V.534a.25.25 0 0 1 .41-.192l2.36 1.966c.12.1.12.284 0 .384L8.41 4.658A.25.25 0 0 1 8 4.466"/>`,`</svg>`].join(``),na=`data:image/svg+xml,${encodeURIComponent(ta)}`,ra=localStorage.getItem(`cx_remote_client_id`);ra||(ra=crypto.randomUUID(),localStorage.setItem(`cx_remote_client_id`,ra));var ia=`Web ${ra.slice(0,8)}`,aa={"Content-Type":`application/json`},oa=La(window.__CX_REMOTE_BASE_PATH__||``),q={auth:Z(`/api/auth`),status:Z(`/api/status`),workspaces:Z(`/api/workspaces`),sessions:Z(`/api/sessions`),sessionsForCwd:(e,t)=>Z(`/api/sessions?nodeId=${encodeURIComponent(e)}&cwd=${encodeURIComponent(t)}`),adoptSession:Z(`/api/sessions/adopt`),codexSessions:(e,t)=>Z(`/api/codex/sessions?nodeId=${encodeURIComponent(e)}&cwd=${encodeURIComponent(t)}&limit=100`),codexSessionPreview:(e,t)=>Z(`/api/codex/sessions/${encodeURIComponent(t)}/preview?nodeId=${encodeURIComponent(e)}`),files:(e,t)=>Z(`/api/files?workspaceId=${encodeURIComponent(e)}&path=${encodeURIComponent(t)}`),session:(e,t=``)=>Z(`/api/sessions/${encodeURIComponent(e)}${t}`),approvals:e=>Z(`/api/approvals?sessionId=${encodeURIComponent(e)}&status=all&limit=50`),approvalResolve:e=>Z(`/api/approvals/${encodeURIComponent(e)}/resolve`),events:(e,t)=>{let n=new URLSearchParams;e&&n.set(`sessionId`,e),t&&n.set(`afterId`,String(t));let r=n.toString();return Z(r?`/api/events?${r}`:`/api/events`)}},J=localStorage.getItem(`cx_remote_session`)||``,sa=localStorage.getItem(`cx_remote_workspace`)||``,ca=[],la=[],ua=[],da=[],fa=[],pa=[],ma=[],ha=[],Y=null,ga={},_a=``,va=``,ya=new Map,ba=``,xa=null,Sa=``,Ca=null,wa=0,Ta=``,Ea=null,Da=null,Oa=!1,ka=0,Aa=new Map,ja=new WeakSet;function X(e){let t=document.getElementById(e);if(!t)throw Error(`Missing element: ${e}`);return t}X(`refresh-icon`).setAttribute(`src`,na),X(`refresh-path-sessions-icon`).setAttribute(`src`,na);async function Ma(){if((await fetch(q.status,{credentials:`same-origin`})).ok)return;let e=$i||prompt(`Access token`)||``,t=await fetch(q.auth,{method:`POST`,credentials:`same-origin`,headers:{Authorization:`Bearer ${e}`}});if(!t.ok)throw Error(await Na(t))}async function Na(e){let t=await e.text();try{return JSON.parse(t).error?.message||t||e.statusText}catch{return t||e.statusText}}function Pa(){let e=new URL(location.href);e.searchParams.delete(`token`),history.replaceState(null,``,e.pathname+e.search+e.hash)}function Fa(e,t){let n=ya.get(t||Ga()?.nodeId||`local`)||va;return n&&(e===n||e.startsWith(`${n}/`))?`~${e.slice(n.length)}`:e}function Ia(e,t){return e.length>t?`${e.slice(0,t-1)}...`:e}function Z(e){return`${oa}${e}`}function La(e){let t=e.replace(/\/+$/,``);return t===`/`?``:t}async function Q(e,t={}){let n=await fetch(e,{...t,credentials:`same-origin`,headers:{...aa,...t.headers||{}}}),r=await n.text();if(!n.ok)try{let e=JSON.parse(r);throw Error(e.error?.message||r)}catch(e){throw e instanceof SyntaxError?Error(r||n.statusText):e}return n.headers.get(`content-type`)?.includes(`application/json`)&&r?JSON.parse(r):r}async function Ra(){let e=await Q(q.status);va=e.homePath,ya.clear(),e.nodes.forEach(e=>{e.homePath&&ya.set(e.id,e.homePath)}),ga=e.codexDefaults||{},qo(e.codexRuntimeDefaults),Ko(`new`,ga),wo(e.eventCursor),X(`status`).textContent=`${e.stats.sessions} managed sessions · ${e.stats.pendingApprovals} approvals · ${e.stats.queuedPrompts} queued`,ca=await Q(q.sessions),J&&!ca.some(e=>e.id===J)&&(J=``),!J&&ca[0]&&(J=ca[0].id),Ya(),await za(),Xa(),await Ka(),Oo()}async function za(){ha=await Q(q.workspaces),ha.length&&(ha.some(e=>e.id===sa)||(sa=ha[0].id),localStorage.setItem(`cx_remote_workspace`,sa),Ba(),await Va(_a))}function Ba(){let e=X(`workspace-root`);e.innerHTML=``;for(let t of ha){let n=document.createElement(`sl-option`);n.setAttribute(`value`,t.id),n.textContent=`${t.nodeName} · ${t.name} · ${Fa(t.path,t.nodeId)}`,e.appendChild(n)}if(ha.length===1){e.hidden=!0;return}e.hidden=!1,e.value=sa}async function Va(e){let t=Ga();if(!t)return;let n=await Q(q.files(t.id,e));sa=n.workspaceId,localStorage.setItem(`cx_remote_workspace`,sa),_a=n.relativePath||``,X(`cwd`).value=n.current,Ha(n),await Ua(n.current,n.nodeId)}function Ha(e){let t=X(`dirs`);t.innerHTML=``,e.relativePath&&t.appendChild(ko(`..`,{size:`small`,className:`dir-button`,onClick:()=>Va(e.parentPath)}));for(let n of e.entries)t.appendChild(ko(n.name,{size:`small`,className:`dir-button`,onClick:()=>Va(n.relativePath)}));t.childElementCount||t.appendChild(jo(`No child directories`))}async function Ua(e=X(`cwd`).value.trim(),t=Ga()?.nodeId){if(!e||!t){la=[],ua=[],Xa();return}let n=++ka;Oa=!0,Xa();try{let[r,i]=await Promise.all([Q(q.sessionsForCwd(t,e)),Q(q.codexSessions(t,e))]);if(n!==ka||X(`cwd`).value.trim()!==i.cwd||Ga()?.nodeId!==t)return;la=r,ua=i.sessions}finally{n===ka&&(Oa=!1,Xa())}}function Wa(e){let t=new Date(e);return Number.isNaN(t.getTime())?String(e):t.toLocaleString()}function Ga(){return ha.find(e=>e.id===sa)}async function Ka(){if(!J){To(),Y=null,da=[],fa=[],pa=[],ma=[],ba=``,oo();return}let e=J;Y?.id!==e&&(ba=``);let t=await Q(q.session(e));e===J&&(Y=t.session,qa(t.session),da=t.messages,fa=t.approvals,ma=t.queue||[],Co(e,t.eventCursor),pa=await Q(q.approvals(e)),e===J&&(oo(),Do(e)))}function qa(e){ca=Ja(ca,e,!0);let t=Ga();la=Ja(la,e,!!(t&&e.nodeId===t.nodeId&&e.cwd===X(`cwd`).value.trim())),Ya(),Xa()}function Ja(e,t,n){let r=e.filter(e=>e.id!==t.id);return n?[t,...r].sort((e,t)=>t.updatedAt-e.updatedAt):r}function Ya(){let e=X(`recent-sessions`);if(e.innerHTML=``,!ca.length){e.appendChild(jo(`No Hub sessions`));return}for(let t of ca)e.appendChild(Za(t,!0))}function Xa(){let e=X(`path-sessions`);if(e.innerHTML=``,Oa){e.appendChild(jo(`Loading sessions`));return}let t=ua.filter(e=>!e.managedSessionId);if(!la.length&&!t.length){e.appendChild(jo(`No sessions in this directory`));return}if(la.length){e.appendChild($a(`Hub-managed`));for(let t of la)e.appendChild(Za(t,!1))}if(t.length){e.appendChild($a(`Codex resume`));for(let n of t)e.appendChild(Qa(n))}}function Za(e,t){let n=ko(``,{className:`session${e.id===J?` active`:``}`,size:`small`,onClick:()=>eo(e)}),r=document.createElement(`span`);r.className=`session-line`;let i=document.createElement(`span`);i.className=`session-name`,i.textContent=e.title,r.append(i,Ao(e.status));let a=document.createElement(`span`);return a.className=`session-path muted`,a.textContent=t?[e.nodeName,Fa(e.cwd,e.nodeId)].filter(Boolean).join(` · `):[`Updated ${Wa(e.updatedAt)}`,lo(e.localId)].filter(Boolean).join(` · `),n.append(r,a),n}function Qa(e){let t=ko(``,{className:`session codex-session`,size:`small`,onClick:()=>to(e)}),n=document.createElement(`span`);n.className=`session-line`;let r=document.createElement(`span`);r.className=`session-name`,r.textContent=e.title;let i=document.createElement(`span`);i.className=`session-action`,i.textContent=`Adopt`,n.append(r,i);let a=document.createElement(`span`);return a.className=`session-path muted`,a.textContent=[e.nodeName,`Updated ${Wa(e.updatedAt)}`,lo(e.localId),e.originator||``].filter(Boolean).join(` · `),t.append(n,a),t}function $a(e){let t=document.createElement(`div`);return t.className=`section-label muted`,t.textContent=e,t}async function eo(e){J=e.id,localStorage.setItem(`cx_remote_session`,J),await uo(e),Ya(),Xa(),await Ka(),$o()}async function to(e){let t=await Q(q.codexSessionPreview(e.nodeId,e.localId));if(t.managedSessionId)throw Error(`Codex thread is already managed by Hub session: ${t.managedSessionId}`);Ea=e,Da=t,no(t),await X(`adopt-dialog`).show()}function no(e){X(`adopt-title`).textContent=e.title,X(`adopt-meta`).textContent=[e.nodeName,Fa(e.cwd,e.nodeId),`Updated ${Wa(e.updatedAt)}`,lo(e.localId),`${e.messageCount} messages`].filter(Boolean).join(` · `);let t=X(`adopt-preview-messages`);if(t.innerHTML=``,!e.messages.length){t.appendChild(jo(`No transcript messages`));return}for(let n of e.messages)t.appendChild(ro(n))}function ro(e){let t=document.createElement(`div`);t.className=`preview-msg ${e.role}`;let n=document.createElement(`div`);n.className=`preview-msg-meta`,n.textContent=`${e.role} · ${Wa(e.createdAt)}`;let r=document.createElement(`div`);return r.className=`preview-msg-content`,r.textContent=e.content,t.append(n,r),t}function io(){Ea=null,Da=null,X(`adopt-preview-messages`).innerHTML=``}async function ao(){!Ea||!Da||(J=(await Q(q.adoptSession,{method:`POST`,body:JSON.stringify({nodeId:Ea.nodeId,threadId:Ea.localId,cwd:Da.cwd,title:Da.title,importTranscript:!0,config:Yo(`new`)})})).id,localStorage.setItem(`cx_remote_session`,J),await X(`adopt-dialog`).hide(),io(),await Ra(),$o())}function oo(){Y?so():(X(`session-title`).textContent=`No Hub session`,X(`session-meta`).textContent=`Create a managed session or adopt an existing Codex thread.`,X(`session-detail`).innerHTML=``),po(),bo(),yo(),fo()}function so(){if(!Y)return;let e=Y.config||{};X(`session-title`).textContent=Y.title,X(`session-meta`).textContent=`${Y.nodeName} · ${Fa(Y.cwd,Y.nodeId)}`;let t=X(`session-detail`);t.innerHTML=``;let n=document.createElement(`div`);n.className=`meta-row`,[[`node`,Y.nodeName],[`status`,Y.status],[`control`,Y.controlLabel||`shared`],[`model`,e.model||`-`],[`permission`,e.permissionMode||`-`],[`search`,e.search?`on`:`off`]].forEach(([e,t])=>n.appendChild(co(e,t))),t.appendChild(n);let r=[[`Hub session`,lo(Y.localId)],[`Codex thread`,lo(Y.codexThreadId)],[`Codex turn`,lo(Y.currentTurnId)],[`lease`,Y.controlLeaseExpiresAt?new Date(Y.controlLeaseExpiresAt).toLocaleTimeString():``],[`error`,Y.lastError||``]].filter(([,e])=>e);if(r.length){let e=document.createElement(`div`);e.className=`runtime-line`,e.textContent=r.map(([e,t])=>`${e} ${t}`).join(` · `),t.appendChild(e)}}function co(e,t){let n=document.createElement(`span`);n.className=`meta-chip`;let r=document.createElement(`strong`);return r.textContent=`${e} `,n.append(r,String(t)),n}function lo(e){return e?e.slice(0,8):``}async function uo(e){let t=ha.filter(t=>t.nodeId===e.nodeId).filter(t=>e.cwd===t.path||e.cwd.startsWith(`${t.path}/`)).sort((e,t)=>t.path.length-e.path.length)[0];t&&(sa!==t.id&&(sa=t.id,localStorage.setItem(`cx_remote_workspace`,sa)),await Va(e.cwd===t.path?``:e.cwd.slice(t.path.length+1)))}function fo(){let e=!!Y,t=Y?.controlOwnerId===ra,n=!!(Y?.controlOwnerId&&!t),r=Y?.status===`running`||Y?.status===`waiting_approval`||ma.length>0;X(`rename`).disabled=!e,X(`runtime`).disabled=!e||r,X(`delete`).disabled=!e;let i=X(`notify`);i.hidden=!e,i.disabled=!e,i.checked=!!(Y&&No(Y.id));let a=X(`claim`);a.hidden=!e||t,a.disabled=!e||n,Bo(a,n?`Controlled by ${Y?.controlLabel||`another client`}`:`Take control`);let o=X(`release`);o.hidden=!e||!t,o.disabled=!t;let s=X(`stop`);s.hidden=!e||!r,s.disabled=!r}function po(){let e=X(`messages`);e.innerHTML=``;for(let t of da)e.appendChild(mo(t));ba&&e.appendChild(ho()),e.scrollTop=e.scrollHeight}function mo(e){let t=document.createElement(`div`);return t.className=`msg ${e.role}${e.kind===`error`?` error`:``}`,t.append(_o(e.role,!!e.metadata?.queued),vo(e.content)),t}function ho(){let e=document.createElement(`div`);return e.id=`streaming-message`,e.className=`msg assistant streaming`,e.append(_o(`assistant`,!1),vo(ba)),e}function go(e){ba+=e;let t=document.getElementById(`streaming-message`);if(!t)t=ho(),X(`messages`).appendChild(t);else{let e=t.querySelector(`.msg-content`);e&&(e.textContent=ba)}let n=X(`messages`);n.scrollTop=n.scrollHeight}function _o(e,t){let n=document.createElement(`div`);n.className=`msg-meta`;let r=document.createElement(`sl-badge`);if(r.className=`role-badge role-main`,r.setAttribute(`variant`,e===`user`?`primary`:e===`assistant`?`success`:`neutral`),r.textContent=e,n.appendChild(r),t){let e=document.createElement(`sl-badge`);e.className=`role-badge queued-badge`,e.setAttribute(`variant`,`neutral`),e.textContent=`queued`,n.appendChild(e)}return n}function vo(e){let t=document.createElement(`div`);return t.className=`msg-content`,t.textContent=e,t}function yo(){let e=pa.filter(e=>e.status!==`pending`),t=X(`approvals`);t.hidden=fa.length===0&&e.length===0,xo(X(`pending-approvals`),fa,!0);let n=X(`approval-history-details`);n.hidden=e.length===0,xo(X(`approval-history`),e,!1)}function bo(){let e=X(`prompt-queue`);if(e.innerHTML=``,e.hidden=ma.length===0,!ma.length)return;let t=document.createElement(`div`);t.className=`muted`,t.textContent=`Prompt queue`,e.appendChild(t),ma.forEach((t,n)=>{let r=document.createElement(`div`);r.className=`queue-job ${t.status}`;let i=document.createElement(`div`);i.textContent=`#${n+1} · ${t.status} · ${t.source} · ${new Date(t.createdAt).toLocaleString()}`;let a=document.createElement(`pre`);a.textContent=t.text,r.append(i,a),e.appendChild(r)})}function xo(e,t,n){if(e.innerHTML=``,e.hidden=t.length===0,!t.length)return;let r=document.createElement(`div`);r.className=`muted`,r.textContent=n?`Pending approvals`:`Approval history`,e.appendChild(r);for(let r of t){let t=document.createElement(`div`);t.className=`approval${n?``:` resolved`}`;let i=document.createElement(`div`);i.textContent=`${r.toolName} · ${r.status}${r.decision?` · ${r.decision}`:``}`;let a=document.createElement(`pre`);if(a.textContent=JSON.stringify(r.input,null,2),t.append(i,a),n){let e=document.createElement(`div`);e.className=`row`,e.append(ko(r.type===`choice`?`Choose 1`:`Allow`,{variant:`primary`,onClick:()=>So(r.id,r.type===`choice`?`0`:`approved`)}),ko(r.type===`choice`?`Cancel`:`Deny`,{variant:`danger`,onClick:()=>So(r.id,r.type===`choice`?`cancel`:`denied`)})),t.appendChild(e)}e.appendChild(t)}}async function So(e,t){await Q(q.approvalResolve(e),{method:`POST`,body:JSON.stringify({decision:t,controlType:`web`})}),await Ka()}function Co(e,t){let n=Number(t);!Number.isFinite(n)||n<=0||n>(Aa.get(e)||0)&&Aa.set(e,n)}function wo(e){let t=Number(e);Number.isFinite(t)&&t>wa&&(wa=t)}function To(){xa&&xa.close(),xa=null,Sa=``}function Eo(){Ca&&Ca.close(),Ca=null}function Do(e){if(xa&&Sa===e)return;To(),Sa=e;let t=q.events(e,Aa.get(e));xa=new EventSource(t,{withCredentials:!0}),xa.onmessage=t=>{let n=JSON.parse(t.data);if(Co(e,t.lastEventId||n.id),e===J&&n.type!==`ready`){if(n.type===`message.delta`){go(String(n.payload?.delta||``));return}if(n.type===`session.deleted`){J=``,localStorage.removeItem(`cx_remote_session`),To(),Uo(Ra());return}if(n.type===`message.created`){let e=n.payload?.message;if(e?.id&&!da.some(t=>t.id===e.id)){da.push(e),e.role===`assistant`&&(ba=``),po();return}}([`approval.created`,`approval.resolved`,`session.control.updated`].includes(n.type)||n.type===`session.updated`&&n.payload?.queuedPrompts!==void 0)&&Uo(Ka())}},xa.onerror=()=>Ho(Error(`Event stream disconnected`))}function Oo(){if(!ca.some(e=>No(e.id))||!(`Notification`in window)||Notification.permission!==`granted`){Eo();return}Ca||(Ca=new EventSource(q.events(void 0,wa),{withCredentials:!0}),Ca.onmessage=e=>{let t=JSON.parse(e.data);if(wo(e.lastEventId||t.id),t.type!==`message.created`)return;let n=t.payload?.message;n?.id&&Lo(n)},Ca.onerror=()=>Ho(Error(`Notification event stream disconnected`)))}function ko(e,t){let n=document.createElement(`sl-button`);if(n.setAttribute(`type`,`button`),t.className&&(n.className=t.className),t.size&&n.setAttribute(`size`,t.size),t.variant&&n.setAttribute(`variant`,t.variant),t.icon){let e=document.createElement(`sl-icon`);e.setAttribute(`slot`,`prefix`),e.setAttribute(`library`,`system`),e.setAttribute(`name`,t.icon),n.appendChild(e)}return e&&n.append(e),n.addEventListener(`click`,()=>$(n,t.onClick)),n}function Ao(e){let t=document.createElement(`sl-badge`);return t.setAttribute(`variant`,e===`idle`?`neutral`:`primary`),t.textContent=e,t}function jo(e){let t=document.createElement(`div`);return t.className=`muted`,t.textContent=e,t}function Mo(){try{let e=JSON.parse(localStorage.getItem(ea)||`{}`);return e&&typeof e==`object`&&!Array.isArray(e)?e:{}}catch{return{}}}function No(e){return Mo()[e]===!0}function Po(e,t){let n=Mo();t?n[e]=!0:delete n[e],localStorage.setItem(ea,JSON.stringify(n))}async function Fo(){if(!(`Notification`in window))throw Error(`Browser notifications are unavailable`);if(Notification.permission!==`granted`){if(Notification.permission===`denied`)throw Error(`Browser notifications are blocked`);if(await Notification.requestPermission()!==`granted`)throw Error(`Browser notification permission was not granted`)}}async function Io(e){let t=Y?.id;if(t)try{e&&(await Fo(),await Ro()),Po(t,e)}finally{fo(),Oo()}}function Lo(e){e.role!==`assistant`||!e.content.trim()||No(e.sessionId)&&(!(`Notification`in window)||Notification.permission!==`granted`||new Notification(zo(e.sessionId),{body:Ia(e.content.trim(),180),tag:`cx-remote-${e.sessionId}`}))}async function Ro(){let e=await Q(q.status);va=e.homePath,e.nodes.forEach(e=>{e.homePath&&ya.set(e.id,e.homePath)}),wo(e.eventCursor)}function zo(e){let t=ca.find(t=>t.id===e)||Y;return t?`${t.nodeName} · ${t.title}`:`CX Remote`}function Bo(e,t){let n=e.querySelector(`sl-icon`);e.textContent=``,n&&e.appendChild(n),e.append(t)}function Vo(e){Ta=e.id,X(`delete-dialog-message`).textContent=`Delete Hub session ${e.title}?`,Uo(X(`delete-dialog`).show())}function Ho(e){let t=e instanceof Error?e.message:String(e);X(`error-message`).textContent=t,Uo(X(`error-alert`).show())}function Uo(e){e.catch(Ho)}function $(e,t){ja.has(e)||(ja.add(e),Wo(e,!0),Promise.resolve().then(t).catch(Ho).finally(()=>{ja.delete(e),Wo(e,!1),fo()}))}function Wo(e,t){e.toggleAttribute(`aria-busy`,t),`disabled`in e&&(e.disabled=t)}function Go(e){return e.querySelector(`sl-button[type="submit"]`)||e}function Ko(e,t){X(`${e}-search`).checked=t.search===!0,X(`${e}-permission-mode`).value=t.permissionMode||`default`;let n=document.getElementById(`${e}-model`),r=document.getElementById(`${e}-reasoning-effort`);n&&(n.value=t.model||`auto`),r&&(r.value=t.reasoningEffort||`default`)}function qo(e){[`new`,`runtime`].forEach(t=>{Jo(`${t}-model`,`auto`,`Default(${e.model})`),Jo(`${t}-reasoning-effort`,`default`,`Default(${e.reasoningEffort})`)})}function Jo(e,t,n){let r=document.querySelector(`#${e} sl-option[value="${t}"]`);r&&(r.textContent=n)}function Yo(e){let t={search:X(`${e}-search`).checked,permissionMode:X(`${e}-permission-mode`).value},n=document.getElementById(`${e}-model`),r=document.getElementById(`${e}-reasoning-effort`);return n&&(t.model=n.value),r&&(t.reasoningEffort=r.value),t}function Xo(e){Ko(`runtime`,e.config||{}),Uo(X(`runtime-dialog`).show())}async function Zo(){Y&&(await Q(q.session(Y.id,`/config`),{method:`PATCH`,body:JSON.stringify({config:Yo(`runtime`)})}),await X(`runtime-dialog`).hide(),await Ra())}function Qo(e){document.body.classList.toggle(`sidebar-open`,e)}function $o(){window.matchMedia(`(max-width: 760px)`).matches&&Qo(!1)}X(`workspace-root`).addEventListener(`sl-change`,e=>{sa=e.currentTarget.value,_a=``,localStorage.setItem(`cx_remote_workspace`,sa),Uo(Va(``))}),X(`root-dir`).addEventListener(`click`,e=>$(e.currentTarget,()=>Va(``))),X(`refresh`).addEventListener(`click`,e=>$(e.currentTarget,Ra)),X(`refresh-path-sessions`).addEventListener(`click`,e=>$(e.currentTarget,()=>Ua())),X(`sidebar-toggle`).addEventListener(`click`,()=>Qo(!0)),X(`sidebar-close`).addEventListener(`click`,()=>Qo(!1)),X(`sidebar-backdrop`).addEventListener(`click`,()=>Qo(!1)),X(`claim`).addEventListener(`click`,e=>$(e.currentTarget,async()=>{Y&&(await Q(q.session(Y.id,`/control`),{method:`PATCH`,body:JSON.stringify({controlType:`web`,ownerId:ra,controlLabel:ia,ttlMs:Qi})}),await Ka())})),X(`release`).addEventListener(`click`,e=>$(e.currentTarget,async()=>{Y&&(await Q(q.session(Y.id,`/control?ownerId=${encodeURIComponent(ra)}`),{method:`DELETE`}),await Ka())})),X(`stop`).addEventListener(`click`,e=>{$(e.currentTarget,async()=>{J&&await Q(q.session(J,`/interrupt`),{method:`POST`}).then(Ra)})}),X(`rename`).addEventListener(`click`,e=>$(e.currentTarget,async()=>{if(!Y)return;let e=prompt(`Title`,Y.title);e&&(await Q(q.session(Y.id),{method:`PATCH`,body:JSON.stringify({title:e})}),await Ra())})),X(`runtime`).addEventListener(`click`,e=>$(e.currentTarget,()=>{Y&&Xo(Y)})),X(`delete`).addEventListener(`click`,e=>$(e.currentTarget,()=>{Y&&Vo(Y)})),X(`notify`).addEventListener(`sl-change`,e=>$(e.currentTarget,async()=>{await Io(e.currentTarget.checked)})),X(`delete-cancel`).addEventListener(`click`,e=>$(e.currentTarget,()=>X(`delete-dialog`).hide())),X(`adopt-cancel`).addEventListener(`click`,e=>$(e.currentTarget,async()=>{await X(`adopt-dialog`).hide(),io()})),X(`adopt-confirm`).addEventListener(`click`,e=>$(e.currentTarget,ao)),X(`adopt-dialog`).addEventListener(`sl-hide`,()=>io()),X(`runtime-cancel`).addEventListener(`click`,e=>$(e.currentTarget,()=>X(`runtime-dialog`).hide())),X(`runtime-save`).addEventListener(`click`,e=>$(e.currentTarget,Zo)),X(`delete-confirm`).addEventListener(`click`,e=>$(e.currentTarget,async()=>{Ta&&(await Q(q.session(Ta),{method:`DELETE`}),await X(`delete-dialog`).hide(),J=``,Ta=``,localStorage.removeItem(`cx_remote_session`),await Ra())})),X(`new-session`).addEventListener(`submit`,e=>{e.preventDefault();let t=e.currentTarget;$(Go(t),async()=>{let e=Ga();if(!e)throw Error(`Select a workspace`);let n=X(`cwd`).value.trim(),r=X(`new-title`).value.trim();J=(await Q(q.sessions,{method:`POST`,body:JSON.stringify({nodeId:e.nodeId,cwd:n,title:r,config:Yo(`new`)})})).id,localStorage.setItem(`cx_remote_session`,J),t.reset(),await Ra(),$o()})}),X(`composer`).addEventListener(`submit`,e=>{e.preventDefault();let t=e.currentTarget;$(Go(t),async()=>{if(!J)throw Error(`Select a session`);let e=X(`composer-text`).value;X(`send-state`).textContent=`Sending`;try{await Q(q.session(J,`/messages`),{method:`POST`,body:JSON.stringify({text:e,ownerId:ra,controlLabel:ia})}),t.reset(),await Ka()}finally{X(`send-state`).textContent=``}})}),Uo((async()=>{await Ma(),await Ra()})());